...
class GaussianDiffusion:
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            # print('t', self._scale_timesteps(t))
            ...
            if exclude_conditional: 
                mse *= masks
                # loss_ssim = skimage.metrics.structural_similarity(np.array(target.detach().numpy()), np.array(model_output.detach().numpy()), channel_axis=2)
                terms["mse"] = sum_flat(mse) / sum_flat(masks)
                print("loss mse", terms["mse"])
                # terms["mse"] += (1 - loss_ssim)
            else:
                # loss_ssim = skimage.metrics.structural_similarity(np.array(target), np.array(model_output),
                #                                                   channel_axis=2)
