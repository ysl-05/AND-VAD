...
class GaussianDiffusion:
    ...
    def training_losses(
        ...
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param noise: if specified, the specific noise to try to remove.
        :param normal_align_weight: weight for the normal alignment loss
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), return_features=True, **model_kwargs)
            
            if isinstance(model_output, tuple):
                pred_noise, features = model_output
            else:
            ...
            if features is not None and "z_sp" in model_kwargs:
                loss_fn = DiffusionLoss(normal_align_weight=normal_align_weight)
                terms["loss"] = loss_fn(
                    pred_noise=model_output,
                    target_noise=target,
                    pred_features=features,
                    z_sp=model_kwargs["z_sp"]
                )
            else:
