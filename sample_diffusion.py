...
def main():
    ...
    for j, (data_group) in enumerate(test_batch):
        ...
        root_image_dir = ''
        frame_id += j_current
        path_image = root_image_dir + str(video_name_current[0]) + '/' + str(frame_id)[-3:] + '.jpg'
        print(path_image)
        # area = object_bbox_image(path_image, mm_det_model)
        # area_list.append(area)
        # print('image %d area '.format(j+3), area)
        ...
        ssim_score = 1
        mse_scores = 0
        # cut_imagetwo = data_crop[0].permute(1, 4, 0, 2, 3)  #0, 4, 1, 2,3
...
def create_argparser():
    defaults = dict(
        ...
        resample_steps=1,  # repeat times : time X step
        test_frames='./path_to/test_data',
        label_file='./data/frame_labels_shanghai.npy',
        start_frame_num=0,
        imagesize=128,
        save_gt=False,
        seed=123,
